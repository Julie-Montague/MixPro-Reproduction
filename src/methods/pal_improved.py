...
@torch.no_grad()
def progressive_alpha_cosine(p: torch.Tensor, y_area: torch.Tensor) -> torch.Tensor:
    """
    Paper-style Eq.(8): alpha = cosine_similarity(p, y_area)
    where y_area is the *area-mixed* label (Algorithm 1).
    """
...
@torch.no_grad()
def progressive_alpha_entropy_from_logits(logits: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """
    ...
    Hmax = math.log(p.size(1))
    a = 1.0 - (H / Hmax)
    return a.clamp(0.0, 1.0)


@torch.no_grad()
def progressive_alpha_maxprob_from_logits(logits: torch.Tensor) -> torch.Tensor:
    """
    Even simpler label-free confidence:
      alpha = max softmax prob
    """
    p = torch.softmax(logits, dim=1)
...
def build_mixpro_labels(
    ...
    logits: torch.Tensor,
    alpha_mode: str = "cosine",   # "cosine" (paper), "entropy", "maxprob"
    progress: float = 1.0,        # optional ramp multiplier in [0,1]
    ...
    # confidence alpha
    with torch.no_grad():
        alpha_mode_ = str(alpha_mode).lower().strip()
        if alpha_mode_ == "entropy":
            alpha = progressive_alpha_entropy_from_logits(logits)
        elif alpha_mode_ == "maxprob":
            alpha = progressive_alpha_maxprob_from_logits(logits)
        else:
            # default: paper-style cosine with y_area
            p = torch.softmax(logits, dim=1)
            alpha = progressive_alpha_cosine(p, y_area)

        alpha = (alpha * float(progress)).clamp(0.0, 1.0)
