...
def ddp_setup():
    ...
    - For torchrun, LOCAL_RANK indexes *within* CUDA_VISIBLE_DEVICES.
      So if CUDA_VISIBLE_DEVICES=3 and nproc_per_node=1, then LOCAL_RANK=0 maps to physical GPU 3.
    """
    if ddp_is_on():
...
def evaluate(model: torch.nn.Module, loader, device: torch.device, rank: int) -> dict[str, float]:
    """
    DDP-safe evaluation: compute SUMs per-rank and all-reduce.
    Returns metrics in percent for top1/top5 (same as your accuracy()).
    """
    model.eval()
...
def call_build_mixpro_labels(**kwargs):
    """
    Signature-safe wrapper for build_mixpro_labels (your pal.py requires `logits`).
    Returns: (y_final, lambda_final, alpha)
    """
...
def main():
    ...
    try:
        ...
        # ------------------ DATA ------------------
        # Treat cfg.data.batch_size as GLOBAL; divide for DDP.
        ...
        grid_h, grid_w, patch_size_px = get_patch_grid_and_size(base)
        ...
        best_top1 = -1.0
        best_epoch = -1
        global_step = 0  # scheduler update-step count
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            # DDP-reducible training stats
            train_loss_sum = 0.0
            train_top1_sum = 0.0
            train_n = 0

            # MixPro diagnostics (alpha/lambda_final + apply-rate)
            mix_alpha_sum = 0.0
            mix_alpha_n = 0
            mix_lam_sum = 0.0
            mix_lam_n = 0
            ...
            for step, (x, y) in enumerate(iterator, start=1):
                ...
                if do_mm:
                    ...
                    with autocast(enabled=use_amp):
                        ...
                        y_soft, lambda_final, alpha = call_build_mixpro_labels(
                            logits=logits,  # REQUIRED by your pal.py
                            y_i=y,
                            y_j=y2,
                            num_classes=int(cfg["model"]["num_classes"]),
                        ...
                        if y_soft.shape != logits.shape:
                            raise RuntimeError(
                                f"MixPro label shape mismatch: y_soft={y_soft.shape} vs logits={logits.shape}"
                            )

                        loss = soft_ce_loss(logits, y_soft)

                    # MixPro diagnostics (only meaningful when do_mm=True)
                    bs = x.size(0)
                ...
                # train accuracy vs original hard labels:
                # NOTE: for MixPro batches this is just a sanity signal (mixed input != pure label).
                with torch.no_grad():
                    acc1 = accuracy(logits, y, topk=(1,))[0].item()  # percent
            ...
            # reduce train stats across ranks so rank0 prints global averages
            ...
            # reduce MixPro diagnostics across ranks
            t_mix = torch.tensor(
                [
                    mix_alpha_sum,
                    float(mix_alpha_n),
